...
Mat4f LightSource::getPosToLightClip() const 
{
	...
	Mat4f resized = Mat4f::perspective(getFOV(), m_near, m_far);
	Mat4f inv = m_xform.inverted();
	return resized * inv;
}
        ...
        prog = new GLContext::Program(
            ...
            FW_GL_SHADER_SOURCE(
                ...
                void main()
                {
					...
					// How did you calculate distance again? Cross-product?
					vec3 l_dir =  positionVarying - lightPosEye;
					...
					float cos_s = clamp(dot(-normalVarying, l_dir), 0.0f, 1.0f);

					// Cosine of the angle between the light normal and the incident lighting direction.
					float cos_l = clamp(dot(lightDirEye, l_dir), 0.0f, 1.0f);

					// Spotlight emission distribution, i.e. the circular cone.

					float cone = max(0, min(1, 4*(dot(normalize(lightDirEye), normalize(l_dir))-cos(lightFOVRad/2.0f))/(1-cos(lightFOVRad/2.0f))));
					//float cone = 1.0f;

					// Multiply them all...
					vec3 shading = lightE * brdf * i_dist * cos_s * cos_l;
					...
					vec3 ndc_c = posLightClip.xyz / posLightClip.w;
					float ndc_depth = ndc_c.z;
					vec2 uv_depth = ndc_c.xy / 2.0f + 0.5f;
					float text_depth = texture2D(shadowSampler, uv_depth).x * 2.0f - 1.0f;

					float shadow = 1.0f;

					if (ndc_depth > text_depth + 0.00001f) {
						shadow = 0.0f;
					}
